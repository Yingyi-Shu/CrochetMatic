...
void Commands::processLCode(GCode *com){
    static bool first_wire_transimission = true;
    if (first_wire_transimission == true)
    {
      Wire.begin();
      //Serial2.begin(9600);
      first_wire_transimission = false;
    }
    if( com->L ){
        float needleIndex = com->L;
        int nID = (int) needleIndex - 1;
        int EState = 0; // default latch state is 0 -> closed
        if (com->hasE()){
          EState = com->E; // check if the latch state is updated and store it in EState
        }
//        if(cN != nID){
          String transmit = "N ";
          transmit.concat(nID);
          ...
          transmit.concat(cE);
          transmit.concat(",");
          transmit.concat(EState);
          char send[32];
          transmit.toCharArray(send, 32);
...
void Commands::processMCode(GCode *com) {
    switch( com->M ) {
        ...
        case 32: // M32 directoryname
            ...
            break;
        case 33: // M32 directoryname (i.e. M23 and <24 combined)
            if(com->hasString()) {
                sd.fat.chdir();
                sd.selectFile(com->text);
            }
